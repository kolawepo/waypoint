...
function extractSessionSnapshot() {
  ...
  .filter(line => !line.includes('ChatGPT can make mistakes'))
  .filter(line => !line.includes('Check important info'))
  .filter(line => !line.includes('function '))
  .filter(line => !line.includes('Resume Instruction'))
  .join('\n');
...
lines.push('Continue from the previous conversation without re-explaining context.');
  lines.push('');
  lines.push('---');
